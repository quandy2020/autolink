...
void DisplayUsage(const std::string& binary, const std::string& command) {
    if (command == "info") {
        std::cout << "usage: autolink_recorder info file" << std::endl;
    ...
    for (char option : options) {
        switch (option) {
            ...
            case 'H':
                ...
                break;
            case 'C':
                std::cout << "\t-C, --cpu-profule\t\t\t\tprofile cpu info"
                          << std::endl;
                break;
            case ':':
                break;
...
int main(int argc, char** argv) {
        ...
        switch (opt) {
            ...
            case 'f':
                if (!autolink::common::PathIsAbsolute(std::string(optarg))) {
                    auto opt_file_abs_path = autolink::common::GetEnv("PWD") +
                                             "/" + std::string(optarg);
                    if (autolink::common::PathExists(opt_file_abs_path)) {
                ...
                for (int i = optind; i < argc; i++) {
                    if (*argv[i] != '-') {
                        if (!autolink::common::PathIsAbsolute(
                                std::string(argv[i]))) {
                            auto opt_file_abs_path =
                                autolink::common::GetEnv("PWD") + "/" +
                                std::string(argv[i]);
                            if (autolink::common::PathExists(
                                    opt_file_abs_path)) {
                                opt_file_vec.emplace_back(opt_file_abs_path);
                            } else {
    ...
    // autolink_recorder info
    if (command == "info") {
        if (file_path.empty()) {
            std::cout << "usage: autolink_recorder info file" << std::endl;
            return -1;
        }
        if (!autolink::common::PathIsAbsolute(file_path)) {
            auto file_path_abs =
                autolink::common::GetEnv("PWD") + "/" + std::string(file_path);
    ...
    if (command == "play") {
        ...
        ::autolink::Init(argv[0], "autolink_recorder");
