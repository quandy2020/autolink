...
bool Recorder::Start() {
    ...
    std::string node_name =
        "autolink_recorder_record_" + std::to_string(getpid());
...
void Recorder::TopologyCallback(const ChangeMsg& change_message) {
    ...
    if (change_message.role_type() != autolink::proto::ROLE_WRITER) {
...
void Recorder::FindNewChannel(const RoleAttributes& role_attr) {
    ...
    auto channel_match_func = [&role_attr](
        ...
        if (std::find(channels.begin(), channels.end(),
                      role_attr.channel_name()) != channels.end()) {
            return true;
        }
