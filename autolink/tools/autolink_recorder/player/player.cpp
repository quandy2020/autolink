...
bool Player::ThreadFunc_Play_Nohup() {
    ...
    while (!is_stopped_.load() && autolink::OK()) {
        if (is_paused_) {
            consumer_->Pause();
        } else {
            consumer_->Continue();
...
void Player::HandleNohupThreadStatus() {
...
bool Player::Start() {
    ...
    while (preload_sec > 0 && !is_stopped_.load() && autolink::OK()) {
        std::this_thread::sleep_for(std::chrono::seconds(1));
        --preload_sec;
    }

    auto delay_sec = play_param.delay_time_s;
    while (delay_sec > 0 && !is_stopped_.load() && autolink::OK()) {
        std::this_thread::sleep_for(std::chrono::seconds(1));
        --delay_sec;
    }
