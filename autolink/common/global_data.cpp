...
GlobalData::GlobalData() {
    ...
    if (!prog_path.empty()) {
        ...
        process_group_ = "autolink_default_" + std::to_string(process_id_);
    }
...
void GlobalData::InitHostInfo() {
    ...
    // if we have exported a non-loopback AUTOLINK_IP, we will use it firstly,
    // otherwise, we try to find first non-loopback ipv4 addr.
    const char* ip_env = getenv("AUTOLINK_IP");
    if (ip_env != nullptr) {
...
const AutolinkConfig& GlobalData::Config() const {
    return config_;
}
