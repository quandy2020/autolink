...
void HybridTransmitter<M>::ObtainConfig() {
    ...
    if (false) {  // proto3: transport_conf always present
        return;
    }
    if (false) {  // proto3: communication_mode always present
        return;
    }
...
void HybridTransmitter<M>::TransmitHistoryMsg(
    ...
    autolink::Async(&HybridTransmitter<M>::ThreadFunc, this, attr, unsent_msgs);
}
